...
### Added
- New `plugin eval` flags: `--allow-real-servers` (with `--mocks record`, also start the plugin's real MCP server processes for servers with no mock), `-j`/`--concurrency <n>` (run up to n agent runs at once, 1-8, default 1), and `--trust-plugin` (assert trust in the plugin's code and eval suite, skipping the first-run trust prompt; for CI)
- New `--accept-command <sha256>` flag for `plugin install` and `plugin update`: accept a marketplace-declared command whose sha256 a previous `--json` run reported as `shownCommand.sha256`

### Changed
- Updated `plugin eval --output-dir` default from `./evals/results/<timestamp>/` to `./<eval dir>/results/<timestamp>/`
- Updated descriptions for `plugin eval`'s `--ablation`, `--eval-dir`, `--mocks`, and `--report` to match the current CLI help text
...
- `import` now accepts `cursor` as a source, alongside `codex` and `gemini`
- New `rm` flags: `--discard-unpushed <commit>@<worktree-id>` and `--force-remove-worktree <worktree-id>` (split `rm` out of the shared `attach|logs|stop|kill|rm` completion block since it now has flags the others don't)
- New `--json` flag for `plugin disable`, `plugin enable`, `plugin install`, `plugin uninstall`, `plugin update`, and `plugin validate`
- New `-y`/`--yes` flag for `plugin install` and `plugin update`
- New `plugin marketplace add` flag: `--claudeai` (add the marketplace claude.ai hosts for you)

### Changed
- Updated `mcp add --header` description: now "Set headers for HTTP/SSE servers" (was "Set WebSocket headers")
- Updated `mcp add-json` description: now "Add an MCP server (stdio, SSE, HTTP, or WebSocket) with a JSON string"
- Updated `ultrareview --timeout` default from 30 to 45 minutes
...
documented hidden ones — all match v2.1.214). Version bump only.
...
No CLI surface changes detected (commands, subcommands, and flags — including
documented hidden ones — all match v2.1.204). Version bump only.
...
### Added
- New `gateway` command: Run the enterprise auth/telemetry gateway (with `--config` flag)
...
- New `--bg` / `--background` flag at top level: Start the session as a background agent and return immediately
- New `mcp login` subcommand: Authenticate with an MCP server (with `--no-browser` flag)
- New `mcp logout` subcommand: Clear stored OAuth credentials for an MCP server

### Removed
- Removed deprecated `--mcp-debug` flag (now returns "unknown option")

## [2.1.175] - 2026-06-12

### Added
- New `--safe-mode` flag at top level: Start with all customizations disabled — useful for troubleshooting a broken configuration. Sets CLAUDE_CODE_SAFE_MODE=1
...
### Changed
- Bumped tracked CLI version to 2.1.168; no flag or command surface changes.
