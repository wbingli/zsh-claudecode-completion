...
### Added
- New `gateway` command: Run the enterprise auth/telemetry gateway (with `--config <path>` flag)
...
- New `--bg`/`--background` flag at top level: Start the session as a background agent and return immediately (manage with `claude agents`)
- New `mcp login` subcommand: Authenticate with an MCP server (HTTP, SSE, or claude.ai connector) (with `--no-browser` flag)
- New `mcp logout` subcommand: Clear stored OAuth credentials for an MCP server

### Removed
- Removed deprecated `--mcp-debug` flag (confirmed removed from CLI)
